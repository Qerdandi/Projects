    ...
    def main(self):
        ...
        for c in range(chap):
            for p in range(page):
                ...
                img_url = f"https://funquizzes.fun//uploads/manga/Amongst%20Us/CHAPITRES/{norm_c}/{norm_p}.jpg"
    ...
    def main_2(self):
        ...
        for c in range(chap):
            while good:
                ...
                # img_url = f"https://scansmangas.me/scans/sasayaku-you-ni-koi-wo-utau/{norm_c}/{norm_p}.jpg"
                img_url = f"https://funquizzes.fun/uploads/manga/Sweet%20Home/{norm_c}/{norm_p}.jpg"
                img_ne = f"{norm_c};{norm_p}.{img_url.rsplit('.', 1)[1]}"

                good = self.dw.download_one_img(img_url, img_ne, self.IMAGES_P)
                page += 1
            
            self.i2p.save_to_pdf(f"sweet_home_{norm_c-15+1}", self.IMAGES_P, self.PDF_P)
            self.tool.remove_images(self.IMAGES_P)
            good = True
            page = 1
